...
//Store first playlist for future use:
let userData = {
  songs: [...allSongs],
  currentSong: null,
  songCurrentTime: 0,
...
// Delete button:
const deleteSong = (id) => {
  // Check if song to delete is currentSong:
  ...
  //remove the song object that matches the id:
  userData.songs = userData?.songs.filter((song) => song.id !== id);
  // rerender the songs, highlight it and set playButtons' text:
  renderSongs(userData?.songs);
  highlightCurrentSong();
  setPlayButtonAccessibleText();
  //check if the playlist is empty and add a reset button:
  if (userData?.songs.length === 0) {
    ...
    resetButton.addEventListener("click", () => {
      ...
      // render songs again alphabeticaly:
      renderSongs(sortSongs());
      // update the playbutton's accessible text:
      setPlayButtonAccessibleText();
